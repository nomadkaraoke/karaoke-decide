...
def validate_locale(en_flat: dict, locale_path: Path) -> list[str]:
    ...
    for key in sorted(en_keys & tr_keys):
        ...
        if isinstance(en_val, str) and isinstance(tr_val, str):
            ...
            if missing_ph:
                issues.append(f"[{locale}] Missing placeholder(s) in '{key}': {', '.join(sorted(missing_ph))}")
            if extra_ph:
                issues.append(f"[{locale}] Extra placeholder(s) in '{key}': {', '.join(sorted(extra_ph))}")

    return issues


def main():
    parser = argparse.ArgumentParser(description="Validate translation files against en.json")
    parser.add_argument(
        "--messages-dir",
        type=Path,
        required=True,
    ...
    if args.locale:
        ...
        locale_paths = sorted(
            p for p in args.messages_dir.glob("*.json") if p.name != "en.json" and not p.name.startswith(".")
        )
