    ...
    async with semaphore:
        ...
        if not full_mode and snapshot_data is not None:
            ...
            if locale_path.exists():
                ...
                print(
                    f"[{target_locale}] Delta mode: {len(delta_keys)} changed keys (of {count_keys(english_data)} total)"
                )
            else:
        ...
        if cache is not None:
            ...
            print(
                f"[{target_locale}] Cache: {stats['hits']} hits, {stats['misses']} misses out of {total_strings} strings"
            )

            if dry_run:
                completed.append(target_locale)
                print(
                    f"[{target_locale}] DRY RUN — would translate {len(uncached_english)} strings via Gemini ({len(completed)}/{total})"
                )
                return True
            ...
            if uncached_english:
                ...
                json_to_translate = json.dumps(subset, ensure_ascii=False, indent=2)
                print(
                    f"[{target_locale}] Sending {len(uncached_english)} uncached strings to Gemini (skipping {len(cached_translations)} cached)"
                )
                ...
                print(
                    f"[{target_locale}] Saved (from cache): {out_path} — Keys: {tr_keys}/{en_keys} ({pct:.0f}%) ({len(completed)}/{total})"
                )
                return True
        elif dry_run:
            flat_to_translate = flatten_keys(json.loads(json_to_translate))
            completed.append(target_locale)
            print(
                f"[{target_locale}] DRY RUN — would translate {len(flat_to_translate)} strings via Gemini ({len(completed)}/{total})"
            )
            return True
        ...
        pct = (tr_keys / en_keys * 100) if en_keys else 0
        print(
            f"[{target_locale}] Saved: {out_path} — Keys: {tr_keys}/{en_keys} ({pct:.0f}%) ({len(completed)}/{total})"
        )
        return True
